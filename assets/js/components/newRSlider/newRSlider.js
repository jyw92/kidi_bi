class newRSlider {
   ...
   * ... (기존 파라미터 설명)
   * @param {boolean} [options.pointTitles=false] - 포인트 타이틀 표시 여부 // ✅ 추가
   * @param {Array<string>} [options.pointTitlesValues=[]] - 포인트 타이틀에 표시될 값의 배열 // ✅ 추가
   */
  constructor(options) {
    const defaults = {
      range: false,
      ...
      subLabels: false,
      subValues: [],
      pointTitles: false, // ✅ 추가
      pointTitlesValues: [], // ✅ 추가
    };
  ...
  init() {
    ...
    this.pointTitles = this.container.querySelectorAll(".slider-point-title"); // ✅ 추가

    if (this.gradientColors.length > 0) {
      const initialColor =
        this.gradientColors[this.options.initialValueIndex] || "#2196F3";
  ...
  createSliderHTML() {
    const scaleHTML = this.options.scale
      ? this.values
          .map(
            (value, index) =>
              `<div class="slider-point" data-index="${index}" style="left: ${
                (index / (this.values.length - 1)) * 100
              }%" tabindex="0" role="slider" aria-valuemin="0" aria-valuemax="${
                this.values.length - 1
              }" aria-valuenow="${
                this.currentIndex
              }" aria-valuetext="${value}"></div>`
          )
          .join("")
      : "";

    const labelsHTML = this.options.labels
      ? this.values
          .map(
            (value, index) =>
              `<div class="slider-label" data-index="${index}" style="left: ${
                (index / (this.values.length - 1)) * 100
              }%">${value}</div>`
          )
          .join("")
    ...
    const subLabelsHTML = this.options.subLabels
      ? this.options.subValues
          .map(
            (subValue, index) =>
              `<div class="slider-sub-label" data-index="${index}" style="left: ${
                (index / (this.values.length - 1)) * 100
              }%">${subValue || ""}</div>`
          )
          .join("")
    ...
    // ✅ 추가: 포인트 타이틀 HTML 생성
    const pointTitlesHTML = this.options.pointTitles
      ? this.options.pointTitlesValues
          .map(
            (title, index) =>
              `<div class="slider-point-title" data-index="${index}" style="left: ${
                (index / (this.values.length - 1)) * 100
              }%;color:${this.options.gradient[index]}">${title}</div>`
          )
          .join("")
      : "";
    ...
    return `
            ...
            <div class="slider-labels-container">
                ${
                  pointTitlesHTML
                    ? `<div class="slider-point-titles">${pointTitlesHTML}</div>`
                    : ""
                } ${
      labelsHTML ? `<div class="slider-labels">${labelsHTML}</div>` : ""
                ...
                ${
                  subLabelsHTML
                    ? `<div class="slider-sub-labels">${subLabelsHTML}</div>`
                    : ""
                }
            </div>
        `;
  ...
  // ... setupEventListeners, _handleDrag*, ... (다른 메서드는 수정 필요 없음)
  setupEventListeners() {
    this.points.forEach((point) => {
      ...
      point.addEventListener("touchstart", this._handleDragStart, {
        passive: false,
      });
    });
  ...
  setActivePoint() {
    // ✅ 수정: pointTitles를 활성화 로직에 포함
    [this.pointTitles, this.labels, this.subLabels].forEach((labelSet) => {
      if (labelSet) {
