...
const ChartConfigManager = () => {
  ...
  const setupSearchFunction = () => {
    ...
    const searchButton = document.querySelector(
      ".column--item .row--item .dialog--btn"
    );
    if (!searchButton) {
    ...
    return /* html */ `
      <div class="form-group row">
          ...
          <div class="row--item">
              ...
              <button type="button" class="dialog--btn">검색</button>
          </div>
          </div>
      </div>
  ...
  const template02 = () => {
    return /* html */ `
            <div class="form-group">
                <div style="display:flex;justify-content:center;align-items:center;min-height:386px;">
                    준비중입니다.
                </div>
            </div>
        `;
    ...
    if (dataChartType === "chart-1" || dataChartType === "chart-2") {
      try {
        ...
        if (gubunType && selectListBox && taskTitle) {
          ...
          taskTitle.value = target.textContent;
