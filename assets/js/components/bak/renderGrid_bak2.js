...
function handleGridItemSwap(draggedIndex, targetIndex, tasks) {
  try {
    // console.log(`아이템 교환: ${draggedIndex} -> ${targetIndex}`);
    ...
    // console.log(`컬럼 타입 교환: ${draggedColumnType} <-> ${targetColumnType}`);
    ...
    if (draggedTask && targetTask) {
      ...
      // console.log('Task 컬럼 업데이트:', {
      //   draggedTask: { id: draggedTask.id, newColumn: draggedTask.column },
      //   targetTask: { id: targetTask.id, newColumn: targetTask.column }
      // });
      ...
      // 교환 완료 이벤트 발생
      window.dispatchEvent(new CustomEvent('gridItemsSwapped', {
        detail: {
...
function moveTaskToColumn(taskId, newColumnType) {
  try {
    ...
    if (taskIndex !== -1) {
      ...
      // console.log(`Task ${taskId} 이동: ${oldColumnType} -> ${newColumnType}`);
...
function swapTasksBetweenColumns(columnType1, columnType2) {
  try {
    ...
    if (task1 && task2) {
      ...
      // console.log(`컬럼 간 Task 교환: ${task1.id}(${columnType2}) <-> ${task2.id}(${columnType1})`);
      
      // 로컬스토리지 업데이트
      saveTasksToLocalStorage(tasks);
      
      // 교환 완료 이벤트 발생
      window.dispatchEvent(new CustomEvent('tasksSwapped', {
        detail: {
...
function saveTasksToLocalStorage(tasks) {
  try {
    ...
    if (tasksString !== existingTasksString) {
      ...
      // console.log("로컬스토리지에 tasks 저장됨:", tasks);
...
function loadTasksFromLocalStorage() {
  try {
    ...
    // console.log("로컬스토리지에서 tasks 불러옴:", tasks);
    return tasks;
  } catch (error) {
...
function renderTwoColumnGrid(grid, uniqueColumns, tasks) {
  const layoutConfigs = {
    ...
    vertical: [
      ...
      { types: ["type01", "type04"], order: ["type01", "type04"] },
      { types: ["type04", "type01"], order: ["type04", "type01"] },
      { types: ["type02", "type03"], order: ["type02", "type03"] },
      { types: ["type03", "type02"], order: ["type03", "type02"] },
  ...
  if (horizontalConfig) {
    ...
    grid.innerHTML = 
      createGridItemHTML(renderOrder[0], tasks) + 
      createGridItemHTML(renderOrder[1], tasks);
  } else {
    grid.style.gridTemplateColumns = `repeat(1, 1fr)`;
...
function createGridItemHTML(columnType, tasks, isFullWidth = false) {
  ...
  return `
    <div class="grid--item" data-grid-type="chart" data-column-type="${columnType}" style="${spanStyle}">
      <div class="chart--item inner--item">
        <h3 class="grid--title" style="border-bottom:2px solid ${color};">${title}</h3>
