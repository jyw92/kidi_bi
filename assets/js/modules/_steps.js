...
function resultLoader() {
  ...
  let percent = 0;
  const animationDuration = 1.2; // 모이는 속도 조절
  const totalItems = items.length;
  const percentIncrement = 100 / totalItems; // 각 아이템당 증가할 퍼센트 (25%)
  ...
  items.forEach((item, index) => {
      ...
      const targetY = 80; // 물속으로 들어가는 y 값 (조정 가능)
      ...
      gsap.to(item, {
          ...
          onStart: () => {
            ...
            cnt.innerText = Math.round(percent);
            console.log("물 애니메이션 시작", percent);
            gsap.to(water, {
          ...
          onComplete: () => {
              if (index === totalItems - 1) {
                  gsap.to(cnt, { innerText: 100, duration: 0.3 });
                  gsap.to(water, { transform: `translateY(0%)`, duration: 0.6, ease: "power2.out" });
              }
          },
      });
  });
